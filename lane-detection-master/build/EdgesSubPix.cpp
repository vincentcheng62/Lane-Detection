...
void extractSubPixPoints(Mat &dx, Mat &dy, vector<vector<Point> > &contoursInPixel,
	std::vector<cv::Point2f> &Points)
{
    int w = dx.cols;
    int h = dx.rows;
	//Points.resize(contoursInPixel.size());
...
#endif
		//Shuffle the vector and only make use of 100 pts in each of the contours
		std::random_shuffle(icontour.begin(), icontour.end());
        for (int j = 0; j < min(100, (int)icontour.size()); ++j)
        {
	...
	int high=100, double alpha=1.0f)
{
	...
	printf("Gaussianblur %f seconds\n", (double)(etime - stime) / (double)cv::getTickFrequency());
	...
	etime = cv::getTickCount();
	printf("postCannyFilter %f seconds\n", (double)(etime - stime) / (double)cv::getTickFrequency());
    // contours in pixel precision
    std::vector<std::vector<cv::Point> > contoursInPixel;
	std::vector<Vec4i> hierarchy;
	//imwrite("zzz_gray.bmp", gray);
	//imwrite("zzz_edge.bmp", edge);
	//imwrite("zzz_mask.bmp", mask);
	edge = edge.mul(mask);
	stime = cv::getTickCount();
    findContours(edge, contoursInPixel, hierarchy, RETR_LIST, CHAIN_APPROX_NONE);
	etime = cv::getTickCount();
	printf("findContours %f seconds\n", (double)(etime - stime) / (double)cv::getTickFrequency());
	stime = cv::getTickCount();
    // subpixel position extraction with steger's method and facet model 2nd polynominal in 3x3 neighbourhood
    extractSubPixPoints(dx, dy, contoursInPixel, Points);
	etime = cv::getTickCount();
	printf("extractSubPixPoints %f seconds\n", (double)(etime - stime) / (double)cv::getTickFrequency());
}
